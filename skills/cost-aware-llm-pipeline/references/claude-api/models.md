...
> **Freshness note (2026-09-22):** vendored from the `claude-api` skill bundled with Claude Code 2.1.280 (that skill's own model table is stamped "cached 2026-06-24"). Claude Opus 5.5 (`claude-opus-5-5`, $4/$20 per MTok) went GA on 2026-09-22 and is the default `opus` in Claude Code 2.1.280, so any "launching / use only when the user names it" marker below is pre-launch wording. Claude Opus 4.1 was retired 2026-08-05.
...
| "cheapest opus", "newest opus", "latest opus" (any attribute phrasing) | `claude-opus-5` (attribute phrasings never select `claude-opus-5-5` while it is marked use-only-when-named - only the literal name does) |
