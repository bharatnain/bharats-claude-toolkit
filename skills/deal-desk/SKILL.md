---
name: deal-desk
description: "Per-deal gate at close — scores ONE proposed deal's margin/risk/terms and routes its discount to a named human approver. Use when an AE has asked for a discount above their authority, a customer has redlined the MSA (uncapped indemnity, MFN, perpetual license-back, missing DPA), or per-deal economics (margin after discount, multi-year payment shape, indemnity exposure) need quantifying and approval routing (AE → Director → VP → CFO/CRO/GC). Operates one deal at a time and NEVER auto-approves — output is a scorecard plus a routing recommendation. Not for designing the discount matrix or approver thresholds this skill applies (use `commercial-policy`); not for front-of-funnel lead lifecycle, lead scoring/routing, MQL/SQL or CRM hygiene (use `revops`); not for board-level bookings/ARR/NRR forecasts (use `commercial-forecaster`); not for rep/pipeline weighted forecasting by deal probability (use `sales-forecast-builder`)."
version: 2.8.0
