...
"""Checkpoint helper for the runbook skills (threat-model, vuln-triage,
vuln-patch, dnr-hunt, dnr-respond).
...
rundir gives every skill run a results/<target>/<timestamp>/ directory
(UTC strftime, so lexical sort == chronological).
...
--key defaults to "phase" (vuln-triage, vuln-patch, dnr-*). Pass --key stage
for threat-model bootstrap. progress.json schema:
...
def _rundir_reusable(run: Path, state: str, scope: str | None) -> bool:
    ...
    if scope:
        ...
        # dir under results/ came from another tool — e.g. an external
        # pipeline writing results/<target>/<ts>/ (with its own outputs and no
        # marker) — and must never be adopted or overwritten.
