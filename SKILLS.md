...
_139 vendored skills (52 always-on, 87 user-invoked via `/name`), 8 agents._

## Always-on skills

_Model-invocable: the description loads on every request._

| Skill | Description |
|---|---|
| `agent-protocol` | Inter-agent communication protocol for C-suite agent teams. Defines invocation syntax, loop preventi… |
| `agent-wrap-up` | Close out (or revisit) a Claude Managed Agent (CMA) build from /launch-your-agent — refresh the over… |
...
| `dnr-hunt` | Proactive threat hunt over web/application/access logs — no alert |
| `dnr-respond` | SECURITY incident-response workup for a lead in hand — a |
...
| `threat-model` | Build a threat model for a target codebase. Three modes: "interview" walks an application owner thro… |
...
| `vuln-triage` | Adversarially triage a batch of raw SECURITY-SCANNER findings — |
| `workos-enterprise-auth` | Use when the user asks for a WorkOS docs URL, term, or dashboard field (Sign-in endpoint, initiate_l… |

## User-invoked skills (`/name`)

_`disable-model-invocation: true` — cost nothing until invoked as `/name`._

| Skill | Description |
|---|---|
| `3-statement-model` | Complete, populate and fill out 3-statement financial model templates (Income Statement, Balance She… |
...
| `advisor-profile` | Cold-start interview that builds and maintains company-context.md — the persistent company/advisor c… |
...
| `audit-xls` | Audit a spreadsheet for formula accuracy, errors, and common mistakes. Scopes to a selected range, a… |
| `board` | Build and open the orchestrator board — what is waiting on you, what is happening now, in-flight bea… |
...
| `code-migration` | Run a large-scale language migration with the six-step process: create the map and the rules, stress… |
...
| `comps-analysis` | Build institutional-grade comparable company analyses with operating metrics, valuation multiples, a… |
...
| `doctor` | Health-check the toolkit setup — settings, plugins, tools — and walk through the exact fixes |
...
| `hallmark` | Anti-AI-slop design review and design-DNA extraction. Use ONLY when the user invokes Hallmark by nam… |
...
| `launch-your-agent` | Build and deploy a Claude Managed Agent (CMA) — an agent hosted in the user's own Anthropic account … |
| `lbo-model` | This skill should be used when completing LBO (Leveraged Buyout) model templates in Excel for privat… |
...
| `setup-repo` | Set up a repository for efficient Claude Code work — detect the stack and its test/lint commands, th… |
...
| `team` | Spin up a maturity-matched multi-agent team to deliver a goal end-to-end, with quality gates enforce… |
...
| `toolkit` | Browse this toolkit — vendored skills grouped by domain + the enabled external plugins |
