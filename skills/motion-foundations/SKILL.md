...
description: Motion tokens, spring presets, performance rules, device adaptation, accessibility enforcement, and SSR safety for React / Next.js using motion/react. Foundation layer — all other motion skills depend on this. Use when setting up motion tokens, spring presets, reduced-motion handling, or SSR-safe animation in React or Next.js. Not for animating a specific UI element — button, modal, toast, stagger list, page transition, scroll-reveal, or layout animation (use `motion-patterns`).
