...
> with `cat` on this skill directory's `bootstrap.md` via Bash.
...
`python3 skills/_lib/checkpoint.py load ./.threat-model-state`

- `status == "absent"` OR `"complete"`, OR `--fresh` in `$ARGUMENTS` →
  **fresh start.** Bash:
  `python3 skills/_lib/checkpoint.py reset ./.threat-model-state`,
...
1. Write tool → `./.threat-model-state/_chunk.tmp` containing the
   stage's output JSON.
2. Bash → `python3 skills/_lib/checkpoint.py save ./.threat-model-state <N> <name> --key stage --from ./.threat-model-state/_chunk.tmp`

**End of run.** After writing `<target-dir>/THREAT_MODEL.md`, Bash:
`python3 skills/_lib/checkpoint.py done ./.threat-model-state 5 --key stage`
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.threat-model-state 1 swarm --key stage --from ./.threat-model-state/_chunk.tmp`
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.threat-model-state 2 synthesize --key stage --from ./.threat-model-state/_chunk.tmp`
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.threat-model-state 3 generalize --key stage --from ./.threat-model-state/_chunk.tmp`
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.threat-model-state 4 gap-fill --key stage --from ./.threat-model-state/_chunk.tmp`
...
2. For each remaining section (`## 2. Assets`, `## 3. Entry points`,
   ...
   - Write tool → `./.threat-model-state/_chunk.tmp` containing that ONE
     section's markdown.
   - Bash:
     `python3 skills/_lib/checkpoint.py append ./.threat-model-state/THREAT_MODEL.md --from ./.threat-model-state/_chunk.tmp`
...
4. Stage-3b sibling locations as candidate leads for `/vuln-scan`.
