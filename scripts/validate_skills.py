    ...
    while i < len(fm_lines):
        ...
        if m and not (line[:1] in (" ", "\t")):
            ...
            if stripped and stripped[0] in (">", "|"):
                ...
                i = j
                continue
            elif (not stripped and i + 1 < len(fm_lines)
                  and fm_lines[i + 1][:1] in (" ", "\t")
                  and fm_lines[i + 1].lstrip().startswith("- ")):
                # block sequence: gather indented '- item' lines
                items = []
                j = i + 1
                while (j < len(fm_lines) and fm_lines[j][:1] in (" ", "\t")
                       and fm_lines[j].lstrip().startswith("- ")):
                    items.append(strip_one_quote_pair(fm_lines[j].lstrip()[2:]))
                    j += 1
                keys[key] = items
                i = j
                continue
                ...
                try:
                    ...
                    keys[key] = [str(x) for x in parsed] if isinstance(parsed, list) else [strip_one_quote_pair(x) for x in stripped[1:-1].split(",") if x.strip()]
                except Exception:  # noqa: BLE001 - not JSON; fall back to a quote-aware split
                    keys[key] = [strip_one_quote_pair(x) for x in re.findall(r'"[^"]*"|\'[^\']*\'|[^,\s][^,]*', stripped[1:-1]) if x.strip()]
            else:
...
def build_catalog(root):
    ...
    skill_rows = []  # (name, desc_cell, model_invocable)
    for skill_md in skill_paths:
        ...
        model_invocable = keys.get("disable-model-invocation") != "true"
    ...
    lines.append("")
    always_on = [r for r in skill_rows if r[2]]
    user_invoked = [r for r in skill_rows if not r[2]]
    lines.append(f"_{len(skill_rows)} vendored skills ({len(always_on)} always-on, "
                 f"{len(user_invoked)} user-invoked via `/name`), {len(agent_rows)} agents._")
    lines.append("")
    lines.append("## Always-on skills")
    lines.append("")
    lines.append("_Model-invocable: the description loads on every request._")
    lines.append("")
    lines.append("| Skill | Description |")
    lines.append("|---|---|")
    for name, desc, _ in always_on:
        lines.append(f"| `{name}` | {desc} |")
    lines.append("")
    lines.append("## User-invoked skills (`/name`)")
    lines.append("")
    lines.append("_`disable-model-invocation: true` — cost nothing until invoked as `/name`._")
    lines.append("")
    lines.append("| Skill | Description |")
    lines.append("|---|---|")
    for name, desc, _ in user_invoked:
        lines.append(f"| `{name}` | {desc} |")
    lines.append("")
    lines.append("## Agents")
    lines.append("")
...
def main(argv):
    ...
    for skill_md in skill_paths:
        ...
        # F6 tiering / scoping keys
        dmi = keys.get("disable-model-invocation")
        if dmi is not None and dmi not in ("true", "false"):
            findings.append(Finding(dir_name, path, "error", "F6",
                                    f"'disable-model-invocation' must be true or false, got '{dmi}'"))
        paths = keys.get("paths")
        if paths is not None and (not isinstance(paths, list) or not paths
                                  or not all(isinstance(x, str) and x for x in paths)):
            findings.append(Finding(dir_name, path, "error", "F6",
                                    "'paths' must be a non-empty YAML list of strings"))
        ctx = keys.get("context")
        if ctx is not None and ctx != "fork":
            findings.append(Finding(dir_name, path, "error", "F6",
                                    f"'context' must be 'fork' if present, got '{ctx}'"))
        if dmi != "true":
            model_invocable += 1
    ...
    print(f"{skill_count} skills, {model_invocable} model-invocable "
          f"(always-on listing ≈ {always_on_chars} chars)")
