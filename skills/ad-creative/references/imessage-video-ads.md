...
Credit: this reference distills the format popularized by Shiv Sakhuja and the Gooseworks team (their ads-skills repo is public but carries no open-source license, so nothing from it is vendored here), who report the format performing strongly on Meta.
...
### Route 1: Off-the-shelf pipeline (fastest)

Third-party agent skills exist that handle rendering, recording, SFX, and stitching end to end (Gooseworks distributes one). Use one to validate the format before building anything custom, after checking its license and what it installs.
