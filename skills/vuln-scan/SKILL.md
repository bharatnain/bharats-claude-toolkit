...
name: vuln-scan
description: >-
  Static source-code vulnerability scan of a whole target directory. Reads the
  tree (and THREAT_MODEL.md if present), spawns one parallel review subagent
  per focus area, and writes VULN-FINDINGS.json + VULN-FINDINGS.md for
  /vuln-triage to verify. Read-only — never builds, runs, or reaches the
  network, and never drops a candidate (triage does the rejecting). Use when
  asked to "scan for vulns", "review this codebase for security issues", "find
  security bugs in <dir>", or as the step between /threat-model and
  /vuln-triage. For a diff-scoped review of pending changes use the built-in
  security review instead; this skill sweeps a whole tree.
...
Findings are static candidates; nothing here is execution-verified, and a
proof-of-concept is out of scope by design.
...
4. Remind: these are **static candidates**, not verified — `/vuln-triage`
   decides which survive.

## Constraints

- **Never execute target code.** No Bash, no builds, no `docker`, no network.
  If the user asks you to "reproduce" or "confirm with a PoC," decline — a
  proof-of-concept needs a sandboxed runtime this skill deliberately lacks.
...
prompts the upstream autonomous vulnerability-discovery harness uses for its
`find` and `recon` stages, applied statically here. The broader category menu, DO-NOT-REPORT
