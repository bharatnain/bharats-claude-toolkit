...
> removed. Upstream this ran as a scheduled weekly agent; here it is a reference workflow you can run on demand ("debrief last week's deals") or wire up as an agent yourself.
...
The output feeds `~/.claude/plugins/config/bharats-claude-toolkit/deviation-log.yaml`. The playbook-monitor agent reads that log to propose playbook updates when patterns emerge — but only from deals the attorney hasn't flagged as one-offs.
...
Read `company-context.md` in full. Extract:
...
Using the repository location from `company-context.md`:
...
2. Identify the applicable playbook section(s) from `company-context.md`.
...
```
[#] [Deal] — [Clause]
Playbook position: [standard position from `company-context.md`]
...
Append a structured entry to `~/.claude/plugins/config/bharats-claude-toolkit/deviation-log.yaml` for each agreement processed.
...
Logged to: ~/.claude/plugins/config/bharats-claude-toolkit/deviation-log.yaml
