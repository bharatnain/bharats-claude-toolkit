...
3. **If using the standard template**: Build the standard structure (Sources & Uses, Operating Model, Debt Schedule, Returns Analysis) following this skill's layout guidance, and populate it with the user's assumptions
...
* **Use the template structure** - Follow the organization of the user's provided template, or the standard section structure described in this skill. Do not invent your own layout.
...
Run the `recalc.py` script from the xlsx skill:
```bash
