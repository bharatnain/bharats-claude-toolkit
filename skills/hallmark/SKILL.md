...
The per-theme axis values live as comments at the top of each theme's tokens block in `site/css/tokens.css` (upstream repo, github.com/Nutlope/hallmark; the vendored theme files under [`references/themes/`](references/themes/) carry the same axis values). When in doubt, name your candidate theme out loud and identify its three axis values; if two of three match the previous output, redirect.
