---
name: vuln-patch
description: Generate candidate fixes for verified SECURITY findings and write
  them as inert diffs a human reviews before applying — never applies anything
  itself. Consumes TRIAGE.json (preferred), VULN-FINDINGS.json, or
  INCIDENTS.json; gives each finding a root-cause-first patch subagent plus an
  independent reviewer that sees only the diff, then writes
  PATCHES/bug_NN/{patch.diff,patch_result.json}, PATCHES.md, and PATCHES.json.
  Use when asked to "fix these vulns", "patch the findings", "generate
  security fixes", or "close the loop on triage". Last leg of
  /vuln-scan → /vuln-triage → /vuln-patch.
...
Third leg of the static pipeline (`/vuln-scan` → `/vuln-triage` → `/vuln-patch`).
...
Invoke with `/vuln-patch <findings-path> [--repo PATH] [--top N] [--id fNNN]
[--fresh]`.

**Arguments** (parse from `$ARGUMENTS`):
- findings path (first positional, required): `TRIAGE.json`,
  `VULN-FINDINGS.json`, `INCIDENTS.json`, or any JSON the `/vuln-triage`
  ingest table recognizes.
- `--repo PATH`: target codebase, read-only (default cwd). Required; the
  skill stops if cited files don't resolve under it.
- `--top N`: patch only the N highest-severity true positives.
- `--id fNNN`: patch only the finding with this id.
- `--fresh`: ignore `./.vuln-patch-state/` checkpoint and start over.
...
permitted only for `python3 skills/_lib/checkpoint.py` (state I/O). `find` is
NOT permitted.

**Write scope.** The Write tool may target ONLY paths under `./PATCHES/` and
`./.vuln-patch-state/`. Never write into `--repo`, never `git apply`, never
...
State persists to `./.vuln-patch-state/` so a fresh `/vuln-patch` session resumes
without re-spawning patch or reviewer subagents.
...
All checkpoint I/O goes
through `python3 skills/_lib/checkpoint.py` (atomic, JSON-validated).
...
**Start of run.** Bash:
`python3 skills/_lib/checkpoint.py load ./.vuln-patch-state`

- `status == "absent"` OR `"complete"`, OR `--fresh` in `$ARGUMENTS` →
  fresh start. Bash:
  `python3 skills/_lib/checkpoint.py reset ./.vuln-patch-state`,
  proceed to Phase 0.
...
**End of every phase N.** Write tool → `./.vuln-patch-state/_chunk.tmp` with the
phase's JSON, then Bash:
`python3 skills/_lib/checkpoint.py save ./.vuln-patch-state <N> <name> --from ./.vuln-patch-state/_chunk.tmp`

**End of run.** After writing `PATCHES.md` and `PATCHES.json`, Bash:
`python3 skills/_lib/checkpoint.py done ./.vuln-patch-state 4`
...
`--id`, `--fresh` from `$ARGUMENTS`. If no findings path, stop and ask.

Every input is treated the same way: the findings carry no runnable
proof-of-concept the skill may fire, so the oracle is a fresh-context
reviewer reading source (Phase 3), never an executed test.

**Checkpoint:** Write tool → `./.vuln-patch-state/_chunk.tmp`:
`{"phase": 0, "args": {repo, top, id, findings_path}}`
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.vuln-patch-state 0 args --from ./.vuln-patch-state/_chunk.tmp`

---

## Phase 1: Ingest and normalize

Same input contract as `/vuln-triage` Phase 1. Normalize every input format to a
...
2. **`VULN-FINDINGS.json`** — read `.findings[]`. Unverified; print
   ...
   /vuln-triage first.` and continue.
3. **`INCIDENTS.json`** (the `/dnr-hunt` / `/dnr-respond` output) — same
   mapping as `/vuln-triage` 1a: one finding per `.incidents[]` entry, nested
   ...
   `Warning: INCIDENTS.json has not been triaged. Consider /vuln-triage first.`
   and continue. No-`file` incidents follow 1c like any other finding.
4. **Crash-report results directory** (`reports/bug_NN/report.json` layout) —
   one finding per `reports/bug_NN/`. Map `report.json` → `description`,
   `crash.crash_type` → `category`, ASAN top-frame → `file`/`line`. Expect
   prose exploitability analysis rather than a clean file/line/category
   shape; patch quality suffers accordingly.
...
- If `--top N`: sort by `severity` HIGH > MEDIUM > LOW then
...
**Checkpoint:** Write tool → `./.vuln-patch-state/_chunk.tmp`:
`{"phase": 1, "findings": [...], "skipped": [...], "repo": ...}`
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.vuln-patch-state 1 ingest --from ./.vuln-patch-state/_chunk.tmp`
...
**Checkpoint per finding:** Write tool → `./.vuln-patch-state/_chunk.tmp` =
`{"id": ..., "bug_nn": "NN", "status": ..., "rationale": ..., ...}`, then Bash:
`python3 skills/_lib/checkpoint.py shard ./.vuln-patch-state <id> --from ./.vuln-patch-state/_chunk.tmp`.
After all findings, write the consolidated phase payload to `_chunk.tmp` then:
`python3 skills/_lib/checkpoint.py save ./.vuln-patch-state 2 generate --from ./.vuln-patch-state/_chunk.tmp`
...
finding. Set `verified: "static_review_only"` on every result regardless of
ACCEPT/REJECT — the label describes the verification class, not the outcome.

**Checkpoint:** Write tool → `./.vuln-patch-state/_chunk.tmp`:
`{"phase": 3, "findings": [...]}`
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.vuln-patch-state 3 review --from ./.vuln-patch-state/_chunk.tmp`
...
For each finding, Write `./PATCHES/bug_NN/patch_result.json`:
...
> re-attacked. Read each diff yourself before applying: check that it fixes
> the root cause rather than the symptom, touches nothing outside the
> affected path, and adds no new input handling.
...
**Step 2 — per finding** (sorted: ACCEPT first, then by severity). Write
`./.vuln-patch-state/_chunk.tmp`:
...
Then `checkpoint.py append ./PATCHES.md --from ./.vuln-patch-state/_chunk.tmp`.
...
```
Patches generated: {N} findings → {M} diffs.
...
These are drafts, reviewed statically only. Read each diff before applying.
```
...
- **Write only under `./PATCHES/` and `./.vuln-patch-state/`.**
...
- **Never execute the target.** No build, no test run, no `docker`. A diff
  that "needs to be tried" is a diff the human tries.

---

## Checking the output

The end-to-end path on a repo with known planted bugs:

```
/vuln-scan <repo>
/vuln-triage <repo>/VULN-FINDINGS.json --repo <repo> --auto
/vuln-patch TRIAGE.json --repo <repo> --top 3
```

Expect one diff per finding under `PATCHES/bug_NN/`, each
`verified: "static_review_only"` with a review verdict and a style score. A
diff whose `variants_checked` is empty, or whose `bypass_considered` reads as
boilerplate, is the one to read first.
...
- **The regression test ships inside the diff** rather than being run. The
  skill cannot execute target code; the test is for the human who applies
  the diff.
...
- **`verified` is the verification class, not pass/fail.**
  ...
  `review`, and must not read it as "this fix was proven."
- **Output shape is stable** (`PATCHES/bug_NN/{patch.diff,
  patch_result.json}`) so a consumer can walk the directory without parsing
  `PATCHES.json` first.
