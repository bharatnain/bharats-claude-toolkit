---
name: vuln-triage
description: Adversarially triage a batch of raw SECURITY-SCANNER findings —
  N independent verifiers re-derive each claim from source and vote, duplicates
  collapse, survivors are re-ranked by derived exploitability and tagged with a
  component owner. Ingests VULN-FINDINGS.json, INCIDENTS.json, SARIF-ish
  scanner JSON, or a markdown report; writes TRIAGE.json + TRIAGE.md for
  vuln-patch to consume. Use when asked to "triage these vulns", "validate
  scanner output", "which findings are real", "kill the false positives", or
  "prioritize the security backlog". Not for product feature requests or
  support tickets. Runs interactively by default; pass --auto to skip the
  interview.
...
- findings path (first positional, required): a JSON file, a directory of
  JSON files, a `VULN-FINDINGS.json`, an `INCIDENTS.json`, a crash-report
  results directory (`reports/bug_NN/report.json` layout), or a markdown
  report.
...
- `--fresh`: ignore any existing checkpoint in `./.vuln-triage-state/` and start
...
`python3 skills/_lib/checkpoint.py` (checkpoint I/O).
...
Phase state persists to `./.vuln-triage-state/` so a fresh `/vuln-triage` session can
...
All checkpoint I/O goes through `python3 skills/_lib/checkpoint.py`
...
State files in `./.vuln-triage-state/`:
...
`python3 skills/_lib/checkpoint.py load ./.vuln-triage-state`

- `status == "absent"` OR `"complete"`, OR `--fresh` in `$ARGUMENTS` →
  **fresh start.** Bash:
  `python3 skills/_lib/checkpoint.py reset ./.vuln-triage-state`,
  then proceed to Phase 0.
- `status == "running"` with `phase_done == N` → **resume.** Read
  `./.vuln-triage-state/phase0.json` through `phaseN.json` **in order** (and any
  ...
  `Resuming from checkpoint: Phase N complete (./.vuln-triage-state/phaseN.json)`,
  and **skip directly to Phase N+1**.

**End of every phase N.** Two tool calls:
1. Write tool → `./.vuln-triage-state/_chunk.tmp` containing the phase's output
   JSON (schema at the tail of each phase section).
2. Bash → `python3 skills/_lib/checkpoint.py save ./.vuln-triage-state <N> <name> --from ./.vuln-triage-state/_chunk.tmp`

**End of run.** After writing `TRIAGE.json` and `TRIAGE.md`, Bash:
`python3 skills/_lib/checkpoint.py done ./.vuln-triage-state 6`
...
**Checkpoint:** Write tool → `./.vuln-triage-state/_chunk.tmp`:
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.vuln-triage-state 0 interview --from ./.vuln-triage-state/_chunk.tmp`
...
- **Directory**: Glob for `**/*.json` and `**/*.jsonl`. Recognized
  ...
  - `reports/bug_*/report.json` or `reports/manifest.jsonl` (fuzzer /
    crash-triage output): one finding per `bug_NN`. Map `crash.crash_type` →
...
repo's own basename, e.g. `api/server.py` with `--repo api`).
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.vuln-triage-state 1 ingest --from ./.vuln-triage-state/_chunk.tmp`
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.vuln-triage-state 2 dedup --from ./.vuln-triage-state/_chunk.tmp`
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.vuln-triage-state 3 verify --from ./.vuln-triage-state/_chunk.tmp`
...
1. Write tool → `./.vuln-triage-state/_chunk.tmp` = that finding's post-tally dict.
2. Bash:
   `python3 skills/_lib/checkpoint.py shard ./.vuln-triage-state <id> --from ./.vuln-triage-state/_chunk.tmp`
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.vuln-triage-state 4 rank --from ./.vuln-triage-state/_chunk.tmp`
...
Then Bash:
`python3 skills/_lib/checkpoint.py save ./.vuln-triage-state 5 route --from ./.vuln-triage-state/_chunk.tmp`
...
1. Write tool → `./.vuln-triage-state/_chunk.tmp` containing ONE finding's section:
...
2. Bash:
   `python3 skills/_lib/checkpoint.py append ./TRIAGE.md --from ./.vuln-triage-state/_chunk.tmp`

Repeat for each true_positive.

**Step 3 — footer.** Write tool → `./.vuln-triage-state/_chunk.tmp` containing the
...
The cheapest end-to-end check is a `/vuln-scan` pass over a small repo you
know, then `/vuln-triage VULN-FINDINGS.json --auto --repo <that-repo>`.
...
`exclusion_rule` or `refute_reasons` should be defensible). Findings whose
`rationale` cites no file:line, or whose `first_links` say "none found",
are the ones to re-read by hand.
...
- **Checkpoints are per-phase JSON**, not conversation state. Resuming a
  ...
  brand-new session pick up from the last completed phase. `./.vuln-triage-state/` is scratch — add to `.gitignore`.
...
- **Bash is allowed narrowly** for `git log` (owner hints), `jq`/`find`
  (ingest), and `python3 skills/_lib/checkpoint.py` (state I/O).
...
- **Crash-report `report.json` ingest is best-effort.** Those reports describe
