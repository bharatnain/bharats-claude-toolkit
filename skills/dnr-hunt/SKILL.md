---
name: dnr-hunt
description: Proactive threat hunt over web/application/access logs — no alert
  in hand. Profiles the corpus with shell pipelines (never reads a log file
  whole), runs a hypothesis-driven hunt loop with a mandatory written ledger,
  confirms suspects in application source, detonates a minimal local PoC to
  earn a confirmed verdict, and writes INCIDENTS.json + INCIDENT_REPORT.md.
  Use when asked to "hunt the logs", "find the campaign", "look for signs of
  compromise", "is there anything in these logs", or "threat hunt". The
  no-alert entry to the detection & response track; /dnr-respond is the
  lead-in-hand entry, and neither one is a production-incident/outage runbook.
...
**Arguments:**
- `$1` = a directory of log files, or a target directory holding both the
  logs and a `config.yaml` describing how to run the app (`logs_dir`,
  `app_command`, `seed_command`, `port`). Defaults to cwd.
- `--repo` = source tree of the application that produced the logs. When `$1`
  is a target directory with a config, this defaults to its `app/` directory.
  Without source, no finding can rise above `suspected` (see the hard rules).
...
3. **Propose, never execute.** No blocking, no account disabling, no
   credential rotation, no config changes — not even against a local copy.
...
the basename of the resolved target/logs path (`$1`, cwd by default), and
`SCOPE` that resolved path. Bash:

```
python3 skills/_lib/checkpoint.py rundir results/TARGET --state .dnr-hunt-state --scope "SCOPE"
```
...
`results/TARGET/<timestamp>/` (UTC, so lexical sort is chronological). That
path is `{RUN}` — every `{RUN}/...` below means substituting this literal
path; double-quote it (and `SCOPE`) in Bash, since paths may contain spaces.
All outputs and state live under `{RUN}`.
...
`python3 skills/_lib/checkpoint.py` (atomic writes, JSON-validated).
...
```
python3 skills/_lib/checkpoint.py save {RUN}/.dnr-hunt-state <N> <name> --from {RUN}/.dnr-hunt-state/_chunk.tmp
```

**Start of run:** `python3 skills/_lib/checkpoint.py load {RUN}/.dnr-hunt-state`
...
`python3 skills/_lib/checkpoint.py done {RUN}/.dnr-hunt-state 5` —
...
1. Resolve `$1`. If it contains a `config.yaml`, read it for `logs_dir`,
   `app_command`, `seed_command`, and `port`. Otherwise treat `$1` itself as
   the logs directory. The corpus is expected to exist already; this skill
...
3. Resolve `--repo` (default: the target dir's `app/` when a config named
   one). List its files; do not read source yet — Phase 3 reads with
   hypotheses in hand, not before.
...
1. Start the app locally per its config: `seed_command` and `app_command`
   are usually written relative to the app's own directory, so run them
   with that directory as cwd rather than assuming the repo root. Seed
   first, then launch the app in the background with a single command that
   prints the PID, e.g.:

   ```
   python3 -c "import os,subprocess; p=subprocess.Popen(['python3','app/app.py'],cwd='<app-dir>',env={**os.environ,'<PORT_ENV_VAR>':'5252'}); print(p.pid)"
   ```

   Poll `curl -s http://127.0.0.1:<port>/healthz` (or whatever the app's
   liveness route is) until it answers. If the default port is busy, pick
   another via whatever override the app documents and record the port you
   used.
...
- from inside `{RUN}`: `/vuln-triage INCIDENTS.json --repo <repo>` then
  `/vuln-patch TRIAGE.json --repo <repo>` — both skills write to the directory
...
## Adapting this to other corpora

- **Any log source:** point `$1` at any directory of access/app/auth logs.
