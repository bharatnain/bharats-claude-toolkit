...
> removed. Upstream this ran as a data-triggered agent; here it is a reference workflow, run after a deal debrief ("check the playbook for drift").

## Purpose

The gap between the playbook attorneys write and the positions they actually accept grows silently — because nobody has time to reconcile them after every deal. This agent watches the deviation log, detects when a position is being overridden consistently, and proposes a specific update to `company-context.md`. The attorney approves or rejects. The playbook stays alive.
...
Both values are configurable in `company-context.md` under `## Playbook monitor settings`:
...
If these fields are absent from `company-context.md`, use the defaults above.

## What it does

### Step 1 — Read the practice profile and log

1. Read `company-context.md` in full. Extract:
...
2. Read `~/.claude/plugins/config/bharats-claude-toolkit/deviation-log.yaml`. Filter out:
...
If no clause crosses the threshold: log the check to `~/.claude/plugins/config/bharats-claude-toolkit/playbook-monitor-log.yaml` and stop. Do not notify the attorney.
...
2. **Current playbook language** (exact text from `company-context.md`)
...
Current language in `company-context.md`:
...
Write all proposals to `~/.claude/plugins/config/bharats-claude-toolkit/playbook-proposals.md`. Overwrite any existing file — stale unreviewed proposals are replaced, not accumulated.
...
*To review: ask Claude to "review the pending playbook proposals"*
...
Notify the attorney via the destination in `company-context.md`:

> Playbook monitor ran — [N] proposed update(s) ready for your review.
> Ask Claude to review the pending playbook proposals when you have a few minutes.
> Proposals: ~/.claude/plugins/config/bharats-claude-toolkit/playbook-proposals.md

Log the run to `~/.claude/plugins/config/bharats-claude-toolkit/playbook-monitor-log.yaml`:
...
- run_at: [ISO datetime]
  ...
  proposals_file: ~/.claude/plugins/config/bharats-claude-toolkit/playbook-proposals.md
```

### Step 5 — Review and approval (triggered when the user asks to review proposals)

When the attorney asks to review the pending playbook proposals:

1. Read `~/.claude/plugins/config/bharats-claude-toolkit/playbook-proposals.md`. If file doesn't exist or is empty: *"No pending proposals. Playbook is up to date."* Stop.
...
[A] Accept — apply proposed language to `company-context.md`
...
5. **Reject / Defer:** log to `~/.claude/plugins/config/bharats-claude-toolkit/playbook-monitor-log.yaml` with reason if given. Do not modify `company-context.md`. A rejected proposal is not re-raised until a new pattern emerges after the rejection date.

6. After all proposals resolved, show summary:

```
Review complete.
[N] accepted and applied to `company-context.md`
[N] rejected
[N] deferred to next cycle
[N] edited and applied

`company-context.md` last updated: [timestamp]
Next playbook check: after [N] more deals are logged
```

7. Archive: rename `~/.claude/plugins/config/bharats-claude-toolkit/playbook-proposals.md` to `~/.claude/plugins/config/bharats-claude-toolkit/playbook-proposals-[YYYYMMDD].md`. The active file is now clear.

## What this agent does NOT do

- Modify `company-context.md` without explicit per-change attorney confirmation
