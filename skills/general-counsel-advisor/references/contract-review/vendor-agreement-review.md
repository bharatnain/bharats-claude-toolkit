...
> removed. Loaded via `routing.md` when a vendor MSA, services agreement, or similar is detected.
...
Read a vendor agreement against the playbook this team actually uses (in `company-context.md`), find every term that deviates, and tell the lawyer what to do about each one — with specific redline language, not vague "consider revising."
...
**Before reading the contract, read `company-context.md`.** If it's missing or still has placeholders, surface this bounce:
...
>
> **Two choices:**
> - Run the `advisor-profile` interview (2 minutes) to configure your profile, then I'll review tailored to YOUR playbook.
...
The playbook in `company-context.md` is the source of truth. It tells you:
...
| Who are we? | Customer / Vendor (this reference assumes customer — flag if not) |
...
Check the "one thing" from `company-context.md` first. If present:
...
For each playbook category in `company-context.md`, find the corresponding contract section and compare.
...
**Playbook says:** [our standard position, quoted from `company-context.md`]
...
**If they won't move:** [the fallback from `company-context.md`, or "escalate to [person]"
...
| Level | Means |
|---|---|
| 🔴 Critical | Don't sign without fixing. A term on the team's "never accept" list in `company-context.md`, or a deal-breaker. |
...
Severity is always applied *against `company-context.md`*. If a term doesn't map cleanly to a playbook position, ask the user which bucket it belongs in and offer to record the answer in `company-context.md`.
...
Check the escalation matrix in `company-context.md` against:
...
**Before proceeding to send redlines to the counterparty:** Read `## Who's using this` in `company-context.md`. If the Role is Non-lawyer:
...
**Before generating a signature envelope or routing for countersignature:** Read `## Who's using this` in `company-context.md`. If the Role is Non-lawyer:
...
**Full memo (default):** As above. Goes in the [CLM] record or the Drive folder from `company-context.md` house-style section.
...
- [ ] `company-context.md` was loaded and quoted — not generic market positions
