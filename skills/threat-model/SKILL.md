...
description: >-
  ...
  codebase are present. All write THREAT_MODEL.md in a shared schema that
  /vuln-scan reads for focus areas and /vuln-triage reads for escalation. Use
  when asked to "threat model this", "build a threat model", "map the attack
  surface", "STRIDE this system", or "what should we be worried about in this
  codebase". Security threat modeling of a system — not a product/launch
  pre-mortem and not a compliance-control review.
...
If the user asks you to validate a threat by running an exploit, decline: a
threat model is derived by reading, and running exploit code needs a
sandboxed runtime this skill deliberately does not have.
...
so downstream consumers (`/vuln-scan` scoping, `/vuln-triage` escalation) do
...
`THREAT_MODEL.md` is the scoping input for `/vuln-scan` (section 3 entry
points and section 4 threats become its focus areas) and the escalation
input for `/vuln-triage` (a finding that maps onto a stated threat outranks
one that doesn't).
