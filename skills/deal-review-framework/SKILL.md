...
description: "Assess a SINGLE in-flight deal against MEDDIC/BANT — scores deal risk, checks the evidence required at the current stage, flags red flags (missing champion, no economic buyer, weak metrics), and gives the rep coaching points and next steps. Use when reviewing one specific opportunity to judge its health and what to do next. Owns the qualification/health lane while the deal is still being worked. Not for the close-time deal-desk gate — discount above AE authority, margin/terms scoring, MSA redlines, or routing approval to a named approver (use `deal-desk`); not for producing sales collateral like decks, one-pagers, demo scripts, or objection-handling docs (use `sales-enablement`); not for adopting/rolling out a methodology org-wide with framework questions, scorecards, or rep training (use `sales-methodology-implementer`); not for mining many won/lost deal notes to find recurring objection patterns (use `objection-pattern-detector`)."
---
